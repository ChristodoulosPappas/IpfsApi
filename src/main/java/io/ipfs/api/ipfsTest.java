    ...
    public static class Node1 extends Thread{
        ...
        public void run(){
            ...
            try {
                ...
                /*
                while(true){
                    Stream<Map<String, Object>> sub = ipfsObj.pubsub.sub("12");
                    System.out.println("SUB STREAM INIT COMPLETED");
                    byte[] decodedBytes;
                    String decodedString = null;

                    System.out.println("WAITTING TO COLLECT");

                    results = sub.limit(1).collect(Collectors.toList());
                    System.out.println("COLLECTED!!!!");

                    String encoded = (String) results.get(0).get("data");
                    decodedBytes = Base64.getDecoder().decode(encoded);
                    decodedString = new String(decodedBytes);
                    //System.out.println(Arrays.toString(ipfs.UnMarshall_Gradients(decodedString)));
                    sub.close();
